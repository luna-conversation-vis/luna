...
class SelectColumn(BaseModel):
    '''base network to predict the relevant column for a select / predicate clause'''
    ...
    def loss(self, model_out, batch):
        ...
        select_targets = batch[2]['select_onehot']
        # column_probabilities = model_out
        # columns_gt = [[j[0] for j in i] for i in select_targets]
        ...
        return self.loss_fn(model_out, select_targets)
    ...
    def accuracy_internal(self, model_out, batch):
        ...
        for index in range(model_out.shape[0]):
            correct = True
            trgs = set(batch[2]['select'][index].tolist())
            # print(trgs)
    ...
    def add_argparse_args(subparser):
        parser = subparser.add_parser(SelectColumn.MODEL_NAME)
